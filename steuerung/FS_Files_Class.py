...
class FS_Files:
	...
	def load_config(self):
		...
		try:
			...
			self.logger.error(f"Error decoding JSON from the config file {self.config_file}.")
			return {}
	...
	def update_config(self, section, key, value):
		...
		if section in self.config:
			...
			self.logger.error(f"Section {section} not found in the config.")
	...
	def add_authorized_user(self, phone_number):
		...
		if 'Benutzer' in self.config:
			...
			self.logger.error("Authorized users section not found in the config.")
